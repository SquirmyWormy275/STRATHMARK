...
def generate_simulation_summary(analysis: Dict[str, Any]) -> str:
    ...
    if analysis.get("heat_variance_seconds") is not None:
        lines.append(f"Heat variance: +/-{analysis['heat_variance_seconds']:.1f}s shared effect")
    lines.append("")
    ...
    lines.append(f"  Tight finish (<10s): {analysis['tight_finish_prob'] * 100:.1f}% of races")
    lines.append(f"  Very tight (<5s):    {analysis['very_tight_finish_prob'] * 100:.1f}% of races")
    lines.append("")
    ...
    for name, pct in sorted_winners:
        lines.append(f"  {name:25s} {pct:5.1f}% ({analysis['winner_counts'][name]:,} wins)")
    lines.append("")
    ...
    if analysis.get("avg_podium_margin_12") is not None:
        ...
        if analysis.get("avg_podium_margin_23") is not None:
            lines.append(f"  2nd - 3rd avg gap: {analysis['avg_podium_margin_23']:.2f}s")
        if analysis.get("photo_finish_pct") is not None:
            threshold = analysis.get("photo_finish_threshold", 0.25)
            lines.append(f"  Photo finish (<{threshold:.2f}s): {analysis['photo_finish_pct']:.1f}%")
