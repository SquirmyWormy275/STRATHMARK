...
def load_and_validate_workbook(path: str) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]:
    """
    ...
    competitor_df.columns = [str(c).strip().lower().replace(' ', '_') for c in competitor_df.columns]
    results_df.columns = [str(c).strip().lower().replace(' ', '_') for c in results_df.columns]

    # Alias common column variants
    _alias_columns(results_df, {
        'time': 'time_s', 'time_seconds': 'time_s', 'raw_time': 'time_s',
        'size': 'size_mm', 'diameter': 'size_mm', 'diameter_mm': 'size_mm',
        'species_code': 'species', 'wood_species': 'species',
        'competitorid': 'competitor_id', 'competitor': 'competitor_id',
        'date': 'result_date', 'event_date': 'result_date',
    })
    _alias_columns(competitor_df, {
        'competitorid': 'competitor_id', 'id': 'competitor_id',
        'competitor_name': 'name', 'fullname': 'name',
    })
    _alias_columns(wood_df, {
        'janka_hardness': 'janka_hard', 'janka': 'janka_hard',
        'specific_gravity': 'spec_gravity',
        'speciescode': 'species', 'species_code': 'species',
    })
    ...
    if 'result_date' in results_df.columns:
        ...
        if n_bad_dates > 0:
            warnings.append(f"Results: {n_bad_dates} rows have unparseable dates (will be treated as undated).")
...
def _check_required_cols(df: pd.DataFrame, required: Set[str], sheet: str, warnings: List[str]) -> None:
...
def write_to_database(
    ...
    if dry_run:
        _log.info("DRY RUN: would write %d competitors and %d results.",
                  len(competitor_df), len(results_df))
    ...
    _alias_columns(results_to_push, {
        'time_s': 'time_seconds',
...
def print_validation_report(
    ...
    if dups:
        ...
        for d in dups[:10 if not verbose else None]:
            print(f"  - Competitor {d['competitor_id']} / {d['event']} / {d['date']}: "
    ...
    if anom:
        ...
        for a in anom[:10 if not verbose else None]:
            print(f"  - Row {a['row_index']}: {a['competitor_id']} / {a['event']} "
...
def main():
    ...
    parser.add_argument('--verbose', action='store_true',
                        help="Show all flagged records (not just first 10).")
    ...
    _log.info("Loaded: %d wood species, %d competitors, %d results.",
              len(wood_df), len(competitor_df), len(results_df))
    ...
    if args.dry_run:
        ...
        _log.info(
            ...
            stats['n_competitors'], stats['n_results'],
        )
