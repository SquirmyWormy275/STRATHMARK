...
## Context
v0.3.0 shipped one coordinated hardening package across the prediction pipeline. Four gaps were closed together because they all undermined the same guarantee: that a mark sheet is defensible in a protest hearing. Freeform Ollama responses fell through to ML whenever the model freestyled a field name — regex parsing was the fragile approach they replaced (session history). The LLM tier existed as a shadow path in `predict_with_llm()`, not wired into the canonical cascade. Monte Carlo ran at 2,000,000 samples without vectorization — slow enough that nobody ran it pre-race (session history). Default variance was a flat 3.0s (auto memory [claude]) and mark rounding used `math.ceil()`, inflating every non-integer-gap mark by ~0.5s on average.
